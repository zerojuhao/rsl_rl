import torch
import torch.nn as nn

class AttentionEncoderBlock(nn.Module):
    def __init__(self, d_obs:int, embedding_dim=64, h=8, map_size=(17,11), map_resolution=0.1, single_obs_dim=78, velocity_estimation=False):
        super(AttentionEncoderBlock, self).__init__()

        if embedding_dim <= 3:
            raise ValueError("embedding_dim must exceed the spatial coordinate dimension (3)")
        if embedding_dim % h != 0:
            raise ValueError("embedding_dim must be divisible by the number of attention heads")

        self.embedding_dim = embedding_dim
        self.h = h
        self.L = map_size[0]
        self.W = map_size[1]
        self.map_resolution=map_resolution
        self.single_obs_dim = single_obs_dim
        self.velocity_estimation = velocity_estimation

        self.register_buffer('pos_encoding', self._create_position_encoding())

        self.cnn = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=3, padding="same"),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.Conv2d(16, 32, kernel_size=3, padding="same"),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.Conv2d(32, (self.embedding_dim - 3), kernel_size=3, padding="same"),
        )

        if self.velocity_estimation:
            self.proprio_linear = nn.Sequential(
                nn.Linear(d_obs, 256),
                nn.ReLU(),
                nn.Linear(256, 128),
                nn.ReLU(),
                nn.Linear(128, self.embedding_dim),
            )
        else:
            self.proprio_linear = nn.Linear(d_obs, self.embedding_dim)

        # 多头注意力模块
        self.mha = nn.MultiheadAttention(embed_dim=self.embedding_dim, num_heads=self.h, batch_first=True)

        # 层归一化
        self.ln_q = nn.LayerNorm(self.embedding_dim)
        self.ln_kv = nn.LayerNorm(self.embedding_dim)
        self.ln_out = nn.LayerNorm(self.embedding_dim)

    def _create_position_encoding(self):
        x_coords = torch.linspace(0, (self.L - 1) * self.map_resolution, self.L)
        x_center = x_coords.mean()
        x_coords = x_center - x_coords

        y_coords = torch.linspace(0, (self.W - 1) * self.map_resolution, self.W)
        y_center = y_coords.mean()
        y_coords = y_center - y_coords
        
        x_grid, y_grid = torch.meshgrid(x_coords, y_coords, indexing='xy')  # (W, L)
        
        pos_encoding = torch.stack([x_grid, y_grid], dim=0)  #(2, W, L)
        return pos_encoding

    def forward(self, map_scans, proprioception):
        B = map_scans.shape[0]
        map_scans = map_scans.view(B, self.W, self.L, 1)

        # 1. 处理地图扫描
        # 提取z值 (高度) 并转换为 channels-first 格式 (B, 1, W, L)
        z_values = map_scans.permute(0, 3, 1, 2)

        # 通过CNN处理z值
        cnn_features = self.cnn(z_values)  # (B, d-3, W, L)

        # 与位置编码和CNN特征拼接
        pos_encoding_batch = self.pos_encoding.unsqueeze(0).expand(B, -1, -1, -1)
        local_features = torch.cat([pos_encoding_batch, z_values, cnn_features], dim=1)  # (B, d, W, L)

        # 重塑为点级特征 (B, L*W, d)
        pointwise_features = local_features.permute(0, 2, 3, 1).contiguous().view(B, self.W * self.L, self.embedding_dim)

        # 2. 处理本体感觉
        proprio_embedding = self.proprio_linear(proprioception.unsqueeze(1))  # (B, 1, d)

        # 3. 多头注意力 (Transformer Encoder Block)
        # 归一化
        q = self.ln_q(proprio_embedding)
        kv = self.ln_kv(pointwise_features)

        # 注意力 + 残差连接
        attn_output, attn_weights = self.mha(
            query=q, # (B, 1, d)
            key=kv,  # (B, L*W, d)
            value=kv
        )
        x = proprio_embedding + attn_output # 残差连接

        map_encoding = self.ln_out(x)

        # reshape to (B, d) & (B, W, L)
        map_encoding = map_encoding.squeeze(1)
        attn_weights = attn_weights.view(B, self.W, self.L)

        if self.velocity_estimation:
            return map_encoding, proprioception[:, -self.single_obs_dim:], attn_weights, proprio_embedding[...,  -3:].squeeze(1)
        else:    
            return map_encoding, proprioception, attn_weights

class AttentionEncoder(nn.Module):
    def __init__(self, d_obs, embedding_dim=64, h=8, map_size=(17,11), map_resolution=0.1, single_obs_dim=78, velocity_estimation=False):
        super(AttentionEncoder, self).__init__()

        # 注意力地图编码模块
        self.encoder = AttentionEncoderBlock(d_obs, embedding_dim, h, map_size, map_resolution, single_obs_dim, velocity_estimation)

    def forward(self, map_scans, proprioception, embedding_only=False):
        # 获取编码
        if self.encoder.velocity_estimation:
            map_encoding, proprioception, attention, velocity = self.encoder(map_scans, proprioception)
        else:
            map_encoding, proprioception, attention = self.encoder(map_scans, proprioception)
        # [B,d], [B,d_obs], [B,W,L]
        # 拼接地图编码和原始本体感觉
        if embedding_only:
            return map_encoding, attention
        else:
            if self.encoder.velocity_estimation:
                combined = torch.cat([map_encoding, proprioception, velocity], dim=-1)  # (B, d + d_obs + vel_dim)
            else:
                combined = torch.cat([map_encoding, proprioception], dim=-1)  # (B, d + d_obs)
            return combined, attention


if __name__ == "__main__":
    d = 64  # MHA维度
    h = 8  # 注意力头数
    d_obs = 78*10  # 本体感觉维度 
    map_size = (17, 11)  # 地图尺寸
    # 创建模型
    model = AttentionEncoder(d_obs, d, h, map_size, 0.1, 78)

    # 创建示例输入
    batch_size = 4
    map_scans = torch.randn(batch_size, map_size[1], map_size[0], 1)  # (4, 11, 17, 1)
    proprioception = torch.randn(batch_size, d_obs)  # (4, 780)

    # 前向传播
    embedding, attention = model(map_scans, proprioception, True)

    print(f"输入地图扫描形状: {map_scans.shape}")
    print(f"输入本体感觉形状: {proprioception.shape}")
    print(f"output embedding shape: {embedding.shape}")
    print(f"output attention shape: {attention.shape}")